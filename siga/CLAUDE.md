...
- `/calendario` — monthly grid of `folgas` (type: 'folga' | 'ferias'). Chefe/admin register via [AusenciaDrawer](src/app/calendario/AusenciaDrawer.tsx) (multi-day selection). [CalendarioClient](src/app/calendario/CalendarioClient.tsx) includes a styled PDF export (`handlePrint`). Admin profiles are excluded from the server selector.
...
- `hydration`: `new Date()` called at render time causes server/client mismatch — always initialise date-dependent state inside `useEffect`.
