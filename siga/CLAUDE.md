...
**Database tables (Supabase).** `processes`, `steps`, `profiles`, `process_shares`, `comunicados`, `folgas`, `notifications`, `audit_logs`, `user_process_types`, `user_step_types`. Schema and RLS policies are in [supabase_migration.sql](supabase_migration.sql) — when schema changes are needed, provide SQL for the user to run in the Supabase SQL Editor.
